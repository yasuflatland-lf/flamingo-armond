...
- **Infrastructure (`internal/repository`, `internal/auth`, etc.) does not import `usecase` or the presentation layer**. Allowing adapter packages to call upward into application logic inverts the dependency arrow and makes it impossible to substitute adapters or test them in isolation; the converse application-to-infrastructure direction is permitted.
...
The backend follows a DDD/layered architecture. Domain is the core: nothing in `internal/domain` or `internal/domain/service` imports any other layer. Outside the core, the enforced rule is the declared `mayDependOn` graph in [`backend/.go-arch-lint.yml`](../../backend/.go-arch-lint.yml), not a strict ring ordering — the application layer imports the infrastructure adapters directly and inverts the dependency at runtime through consumer-defined narrow interfaces (see below).
