...
**Shared helpers must not embed a layer prefix wrap.** A helper that serves multiple callers (e.g. `usecase.AdminGate.Require`, `authorizeCardgroupOrBadInput`) MUST NOT call `eris.Wrap` with a *fixed* prefix hardcoded inside the helper body. The prefix that travels with the wrap must come from the caller: either passed as an argument (`AdminGate.Require(ctx, "usecase: admin role: check admin")`) or applied by a companion classifier the caller invokes (historically the split `requireAdmin` + `wrapAdminGateError` pair, since consolidated into `AdminGate`). A helper-internal fixed-prefix wrap displaces the caller-specific module attribution from the chain, making the logged `error_chain` attribute point at the helper's name instead of the file that actually performed the operation. See [`error-classifier-helper-pass-through-with-caller-prefix.md`](../../docs/backend/error-wrapping/error-classifier-helper-pass-through-with-caller-prefix.md) for the full pattern, the consolidated `AdminGate.Require` form, and the double-wrap failure mode.
