...
**The `root.stack` element type depends on whether the test handler does a JSON round-trip.** A test that captures via `slog.NewJSONHandler(&bytes.Buffer{}, ...)` then `json.Unmarshal`s the buffer sees `root.stack` as `[]any` (every JSON array decodes into `[]any` regardless of element type). A test that captures `slog.Record.Attrs` directly into a `map[string]any` (e.g. a `chanLogHandler` that walks `r.Attrs` and stores `v.Any()`) sees `root.stack` as the native Go type that `eris.ToJSON` returned — `[]string`. A `[]any` cast assertion against the native shape silently widens to a `nil` slice (the type assertion fails, the comma-ok form swallows it) and the `len == 0` branch fires for what is actually a populated stack. Match the assertion to the capture path: JSON round-trip → `[]any`, direct attr capture → `[]string`. Reference: `backend/internal/auth/middleware_test.go` (`assertRichErrorChain` uses `[]string` because `chanLogHandler` skips JSON).
