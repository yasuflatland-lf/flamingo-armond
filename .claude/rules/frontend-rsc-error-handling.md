...
This is why `app/<route>/error.tsx` cannot rescue layout-level throws — the same constraint as the `Gotchas encountered` entry in `frontend/CLAUDE.md` ("`app/<route>/error.tsx` does not catch errors from `app/layout.tsx`").
