...
**Filter test FILES by path, not the bare substring `test`.** When the affordance is keyed by a `data-testid` (e.g. a `catalog-view-{id}` View link), `… | grep -v test` is self-defeating: the `data-testid` attribute literally contains the substring `test`, so the pipe drops the very production line you are verifying and reports a false "dead" zero. Exclude test files by extension instead — `grep -v '\.test\.'`, or `--include='*.tsx' --exclude='*.test.tsx'` — so the real consumer wire stays in the result set. (The earlier render-gating example greps for prop names like `revealed`/`onReveal`, which rarely collide with `test`; a testid-keyed reachability check always does.)
...
**Disable-gating loading props are the same shape (#544).** The rule covers any prop that gates *behaviour* on a consumer-supplied value, not only render-branch props. When a shared component extracts a `busy`/`importing` flag that disables a control while a mutation is in flight, the isolated shared-component test passes the flag explicitly (`importing={true}`) and proves the component *honours* it — but says nothing about whether the wrapper threads `useMutation`'s `loading` into it. A wrapper that hardcoded `importing={false}` or mis-spelled the prop would leave the control enabled mid-flight in production with a green isolated test. The wiring proof is a wrapper-level in-flight test: keep the mutation pending (`delay: Infinity` on the MockedProvider response) and assert the control is disabled. Worked example: `BatchImportWizard`'s `importing` prop is exercised in isolation in `batch-import-wizard.test.tsx`, but the `loading → importing` wiring is proven only by the in-flight test in `cardgroup-batch-import-form.test.tsx`. This is the consumer-wiring half of the extraction seam in [`docs/frontend/typescript-conventions/shared-stateful-component-extraction-seam.md`](../../docs/frontend/typescript-conventions/shared-stateful-component-extraction-seam.md).

### Query under-selection leaves a field `undefined` at the consumer

The render-gating gap above is a *prop*-wiring failure; the GraphQL data layer has the same shape. A consumer that reads `result.x.kind` only sees a real value if the **query selected `kind`**. Omit the field from the `graphql()` selection set and Apollo returns `undefined` for it at the read site — even though the schema types the field non-null — so a branch like `isWarningKind(err.kind)` silently takes its default arm for *every* row. An isolated component test that hand-builds a result object with `kind` present passes; the production query that never requested `kind` is the actual gap, exactly like a prop default that papers over a missing wire.

The check is a selection-set grep, not a prop grep: confirm every field a consumer reads is in the query that feeds it. When you discover a missing field and add it to the selection set, two follow-on obligations fire (codegen re-run + MockedProvider mock sync) — see [`docs/frontend/codegen.md` § "Changing a `graphql()` selection set"](../../docs/frontend/codegen.md#changing-a-graphql-selection-set).

Worked example (issue #685 frontend companion): `BatchImportWizard.isWarningKind` reads `err.kind`, but the validate-step `ValidateCardImportQuery` selected only `errors { line message }`, so on the preview path `kind` was always `undefined` and every error rendered with the blocking (default) styling — a fidelity gap no isolated wizard test caught (the import-result query already selected `kind`). The fix added `kind` to the validate selection.
