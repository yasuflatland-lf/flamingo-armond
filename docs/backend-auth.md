...
The Supabase Custom Access Token Hook (`public.custom_access_token_hook`) joins `public.user_roles` at JWT mint time and emits `app_metadata.role = "admin"` into the access token. The Next.js middleware (`frontend/src/lib/supabase/middleware.ts`) reads this via `supabase.auth.getClaims()` and forwards `isAdmin` as the `x-user-is-admin` request header; the root layout reads that header and passes `isAdmin` to `AppShell` without a GraphQL round-trip. See [`docs/backend/custom-access-token-hook.md`](backend/custom-access-token-hook.md) for the design decisions (join-at-mint vs sync-trigger, fail-closed on malformed events, stale-claim removal, canonical return shape, DROP-auto-revoke, operator precondition for the down migration). The consumer side is documented in [`docs/frontend/auth-supabase.md`](frontend/auth-supabase.md).
