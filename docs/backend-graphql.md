...
**No silent failures in the lexer.** `Lex`, `lexRun`, `lexWord`, and `recoverLineForUnrecognized` all share the same `err != io.EOF` guard: any `ReadRune` error that is not `io.EOF` is recorded via `l.Error("read: " + err.Error())` so the failure surfaces as a structured `ValidationError`. Invalid UTF-8 or transient I/O on the underlying `strings.Reader` therefore produces a visible parse error — never an empty-but-successful result. When `Lex` encounters an unrecognised rune (neither an ASCII headword character nor a definition opener), it hands that rune to `recoverLineForUnrecognized(first rune) (string, int)` (`backend/internal/textdic/lexer.go:192`), which consumes the rune plus the rest of the malformed line and returns two values: the recovered snippet, which `Lex` stores on the emitted `parseError` so it reaches `ValidationError.Snippet` (`lexer.go:119-126`), and the token to emit. That token is `NEWLINE` on both exits — after a line terminator (advancing `lineNo`) and at EOF when the malformed line is the last in the payload, where `lineNo` deliberately stays put because no later token needs attributing and the next `Lex` call reports EOF on its own (`lexer.go:201-204`). Lines after the bad one therefore continue to parse, and partial nodes accumulated up to that point are preserved. Note what does *not* happen: the emitted `NEWLINE` does not trigger the `error NEWLINE` recovery rule at `grammar.y:67`. `grammar.y:74` declares a blank-line production, `entry: NEWLINE { $$ = node{} }`, so the token is shiftable in the current state and the parser never enters error mode — recovery works because the lexer already swallowed the malformed line. Apply this pattern to any new lexer added under `internal/`; the rationale is in [`docs/backend/library-gotchas/goyacc-lexer-recovery-via-newline.md` § "Why the lexer emits NEWLINE"](backend/library-gotchas/goyacc-lexer-recovery-via-newline.md#why-the-lexer-emits-newline).
