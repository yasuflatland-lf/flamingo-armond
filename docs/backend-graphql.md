...
**Cap-checking is symmetric, by contrast.** Both `validateCardImport` and `importCards` run the shared `validateImportPayloadSize` (1 MiB decoded) and `validateImportRows` (5,000-row + per-side 500-grapheme) helpers, so a payload that previews as valid cannot fail the caps at commit time. The two whole-payload caps do differ in *channel* between the two operations — see the schema descriptions in `schema/card_import.graphql` — but not in *verdict*. The asymmetry above is specific to dedup; do not generalise it to the caps. That symmetry is itself conditioned on `textdic.Process` returning non-empty, edge-trimmed sides — the preconditions and the trigger for adding a code defense if a second input source is wired in are in [`docs/backend/library-gotchas/preview-commit-validation-parity.md` § "The parity is conditioned on two textdic output properties"](backend/library-gotchas/preview-commit-validation-parity.md#the-parity-is-conditioned-on-two-textdic-output-properties).
