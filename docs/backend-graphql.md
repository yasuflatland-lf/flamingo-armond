...
**Dedupe is asymmetric: `importCards` dedupes, `validateCardImport` does not.** `importCards` runs dedup and surfaces dropped rows as `CardImportError` entries with `Front`/`Back` populated. `validateCardImport` runs `textdic.Process` directly and surfaces only parser-level syntax errors — those entries never carry `Front`/`Back`. The asymmetry is a usecase-layer fact: the import path calls `dedupeParsedWords` from `(*cardImportUsecase).Import` (`backend/internal/usecase/card_import.go`) and the validate path, `(*cardImportUsecase).Validate` in the same file, does not. It does **not** reach the resolver layer, where both paths share one mapper — `toCardImportErrors` (`backend/graph/resolver/validation.go`), reached on the validate path via `toCardImportValidationResultModel` in the same file — which maps `Snippet`, `Front` and `Back` through `nilIfEmpty` unconditionally. A future change that adds dedup to `validateCardImport` therefore needs no resolver edit: the fields map to `null` while they stay empty and would start carrying values on their own.
