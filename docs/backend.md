...
- Go version is pinned via mise (`backend/mise.toml`). CI resolves Go through `jdx/mise-action` with `working_directory: backend`.
...
- **`bootstrapAuthSchema` fixture is required before migration** — migrations include a trigger that references `auth.users`. In production Supabase provides this schema, but the Postgres test container does not. Tests must call `bootstrapAuthSchema` to create the `auth` schema and `auth.users` table before applying migrations. The same bootstrap SQL is mirrored (copied, not shared) in `tools/schemaspy/auth-stub.sql` for the ER-chart CI workflow; deduplication is deferred, so changing the bootstrap requires updating all five copies (the four test files and the stub). See [`docs/ci.md` - "ER-chart workflow"](ci.md#er-chart-workflow) for the full context.
