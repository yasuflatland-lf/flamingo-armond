...
A domain service that applies session-local ordering (interleave,
...
- `backend/internal/repository/card_due.go` — `findDueCardsOn` fetches the
  review and new windows as independent `LIMIT`
  selections, returning up to `2*limit` rows and making the caller's truncate
  load-bearing.
