...
3. **Typed reason constant** — pair the wrapper with a `BadUserInputReason` constant so the discriminator string the frontend branches on lives in exactly one place and is never stringly-typed at the call site.
