...
Worked example: adding `20260614000000_add_master_tables` (the Master Catalog tables) shifted both per-migration tests up by one — `TestCardsPositionDownUpRoundtrip` went `Steps(-4)` → `Steps(-5)` and `TestUsersVersionDownUpRoundtrip` went `Steps(-5)` → `Steps(-6)`. Adding `20260615000000_add_learn_display_mode_to_user_preferences` directly on top shifted both again — `TestCardsPositionDownUpRoundtrip` to `Steps(-6)` and `TestUsersVersionDownUpRoundtrip` to `Steps(-7)`. Adding `20260616000000_add_user_card_fsrs_card_id_index` on top shifted them once more — `TestCardsPositionDownUpRoundtrip` to `Steps(-7)` and `TestUsersVersionDownUpRoundtrip` to `Steps(-8)`. See `backend/internal/database/cards_position_roundtrip_test.go` and `users_version_roundtrip_test.go`.

A column- or table-shaped migration brings its **own** roundtrip test that uses `Steps(-1)` because its target *is* HEAD, and that test stays at `-1` only until a further migration lands on top of it. The "new migration leaves its own `-1` test alone and shifts every older sibling up by one" asymmetry holds **only when the new migration itself adds a `-1` anchor test**.

A migration with no column or table to assert rolled-back — a **pure-index migration** like `20260616000000_add_user_card_fsrs_card_id_index` — adds **no** per-migration roundtrip test of its own (there is no dropped column/table to check). It therefore provides no new `-1` anchor and instead shifts *every* existing per-migration test up by one, **including the previous HEAD anchor**. That is exactly why `TestLearnDisplayModeDownUpRoundtrip` (`backend/internal/database/learn_display_mode_roundtrip_test.go`), which sat at `Steps(-1)` as the `20260615000000_...` anchor, moved to `Steps(-2)` when the FSRS `card_id` index migration landed on top. After a pure-index migration there is **no** per-migration test at `-1`: its down path is covered by the generic `TestMigrateDownUpRoundtrip` (full `Down()` / re-`Up`), and its up path by an index-existence assertion (`backend/internal/database/user_card_fsrs_card_id_index_test.go`, which queries `pg_indexes` for the `USING btree (card_id)` shape).
