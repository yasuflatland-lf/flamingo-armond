...
The same pattern works for any `Ensure-by-natural-key` repository method on a table without a matching UNIQUE constraint. Two arguments are enough up to a 64-bit composite key; for wider keys, hash the concatenation explicitly with `pg_advisory_xact_lock(hashtextextended(?, 0))`.
