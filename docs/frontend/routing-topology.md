...
Why `isUserOnboarded` is the highest signed-in priority: a user whose `displayName` was nulled by an admin (or who completed OAuth but never finished `/onboarding`) would otherwise land on `/learn/{lastViewedCardgroup}` or `/cardgroups` with an empty profile — visible to other users — and have no in-product affordance to fix it. The gate sits ahead of the cardgroup branches so the empty-`displayName` state is unreachable via any HomePage-funnel entry. `/onboarding` and `/onboarding/start` each call `isUserOnboarded` on their own as well — see their rows in the table above. (The `/login` funnel is not a separate path — an already-signed-in `/login` visitor redirects to `/` and therefore runs the chain; see the `/login` row in the table above.)

The HomePage branch is a funnel guard, not the global invariant: it only runs for visitors who pass *through* `/`. The deep-link path — a signed-in user typing `/learn/{id}`, `/cardgroups`, `/catalog`, `/stats`, `/profile` or an admin URL directly — is closed by the **middleware** display-name gate, which redirects to `/onboarding` before any RSC work on every route the matcher covers. The middleware is the global invariant; the HomePage / `/onboarding` / `/onboarding/start` branches remain because they also pick a *destination*, which the gate does not. See [`onboarding-gate.md` § "The middleware gate closes the deep-link path"](./onboarding-gate.md#the-middleware-gate-closes-the-deep-link-path) for the gate's exemption list, its signed-cookie fast path, and its fail-open contract.
