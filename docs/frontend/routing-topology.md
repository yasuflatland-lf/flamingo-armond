...
Why `isUserOnboarded` is the highest signed-in priority: a user whose `displayName` was nulled by an admin (or who completed OAuth but never finished `/onboarding`) would otherwise land on `/learn/{lastViewedCardgroup}` or `/cardgroups` with an empty profile — visible to other users — and have no in-product affordance to fix it. The gate sits ahead of the cardgroup branches so the empty-`displayName` state is unreachable via any HomePage-funnel entry. It is a funnel guard, not a global invariant: the branch only runs for visitors who pass *through* `/`, so a signed-in user who deep-links straight to `/learn/{id}`, `/cardgroups`, or any other surface that does not run the predicate itself never evaluates it and can still reach that surface with an empty `displayName`. The two exceptions are `/onboarding` and `/onboarding/start`, which each call `isUserOnboarded` on their own — see their rows in the table above. (The `/login` funnel is not a second such path — an already-signed-in `/login` visitor redirects to `/` and therefore runs the chain; see the `/login` row in the table above.) Closing the deep-link path would require a global display-name guard in the middleware or the app layout, which is a separate design decision.
