...
The asymmetry reflects the information hierarchy: the mobile `GlobalFAB` and the per-page add-card affordances remain the global, frequent path; cardgroup creation is a setup-level operation accessible only when creating a card (picker) or managing the cardgroup list (/cardgroups). This surfaces the card → cardgroup parent-child relationship in the interaction flow.
...
- `--brand-primary` (coral `#FE7F70` via OKLCH) — primary CTAs only: the `+ Card` FAB, primary form Save buttons, and the sonner toast `actionButton` (the Undo button on delete toasts). Not for body text, links, or hover states. The sonner `actionButton` is included because it is the sole recovery affordance for a committed delete — it must be unambiguously distinct from the toast body and the `cancelButton`. Apply via `classNames.actionButton: "group-[.toast]:bg-brand-primary group-[.toast]:text-brand-primary-foreground"` in the `<Toaster>` component options.
