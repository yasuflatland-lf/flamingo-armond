...
- `--brand-primary` (coral `#FE7F70` via OKLCH) — primary CTAs only: `+ Card` FAB, the `+ Card` desktop nav button, primary form Save buttons, and the sonner toast `actionButton` (the Undo button on delete toasts). Not for body text, links, or hover states. The sonner `actionButton` is included because it is the sole recovery affordance for a committed delete — it must be unambiguously distinct from the toast body and the `cancelButton`. Apply via `classNames.actionButton: "group-[.toast]:bg-brand-primary group-[.toast]:text-brand-primary-foreground"` in the `<Toaster>` component options.
