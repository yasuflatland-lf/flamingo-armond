...
- **RSC pages** (`page.tsx`): all pages (including login and admin pages) read session status from the middleware-forwarded `x-auth-status` header via `readAuthContext`; none call `getUser()` directly. Pages that call `gqlFetch` must log only `{ name: err instanceof Error ? err.name : "unknown" }` — `err.message` of a GraphQL error can echo user-authored input and must not appear in operator logs.
