...
- **RSC pages** (`page.tsx`): standard pages read session status from the middleware-forwarded header and have only the `gqlFetch` error path — log `{ name: err instanceof Error ? err.name : "unknown" }`. Admin and login pages also call `getUser()` directly and must log only `{ name: authErr.name }` for the auth-error path. The `err.message` of a Supabase auth error may contain user-identifying information (e.g. the email tied to the session token). The `err.message` of a GraphQL error can echo user-authored input. Neither belongs in operator logs.
