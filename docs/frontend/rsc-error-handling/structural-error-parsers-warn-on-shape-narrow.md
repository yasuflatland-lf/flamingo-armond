...
**Application example:** `parseGqlErrors` in `frontend/src/lib/apollo/graphql-errors.ts` is a single-field parser (it reads only `extensions.code`) that wraps its `JSON.parse` step in a `try/catch` and emits a `console.warn` when the payload is malformed — exactly the shape described above. `isUnauthenticatedGraphQLError` and `isForbiddenGraphQLError` are thin wrappers over `parseGqlErrors` that apply a single-key discriminator check; neither carries a multi-field payload, so the warn gate inside the loop body is not reached there. New helpers that read two or more extension fields after a positive discriminator must add the inline warn described in this rule.
