...
The required form surfaces callers that forgot to wire the prop (compile error: "returnTo is missing") rather than silently defaulting to `undefined`. Reference: `frontend/src/app/cardgroups/new/new-cardgroup-client.tsx` (`returnTo: string | null`) after a review finding that the optional form allowed callers to skip the prop and lose the sanitized redirect value without any error. A second worked example is `AdminUserListItem` in `frontend/src/app/admin/users/admin-user-row.tsx` — `displayName`, `bio`, and `avatarUrl` are required `string | null` (not optional `?: string | null`) so the construction sites in `admin-users-client.tsx` (the listing-row mapper and the edit-sheet lazy-query mapper) cannot silently omit a field. The mappers explicitly normalize fragment-masking results with `?? null` to satisfy the required-nullable contract. The required-vs-optional principle is independent of which nullable type is used: it applies equally to `string | undefined` — for example, `frontend/src/app/providers.tsx` types its CSP `nonce` prop as `nonce: string | undefined` (required-nullable) rather than `nonce?: string` (optional), so a call site that forgets to wire the nonce fails to compile rather than silently passing `undefined` unnoticed.
