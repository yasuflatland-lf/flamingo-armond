...
`MasterCardgroupRepository.EnsureByName` commits in its own transaction, before the transaction that upserts and prunes the cards is opened. A first sync for a deck name that fails anywhere inside that second transaction therefore leaves a master cardgroup row behind with zero cards. The row is created unpublished, so it is `DRAFT` and reachable only through the admin catalog — no learner sees it — and the next successful sync for the same name heals it by upserting the cards into that same row. An empty deck appearing after a failed run is this side effect, not a separate defect.
...
- **Anything else waits `2^(attempt-1)` seconds, capped at 5 seconds** — a `5xx` without a usable header, or a transport failure that produced no response at all.
