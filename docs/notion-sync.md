...
Front matching on this path is case-**insensitive** (`master_cards.front` is `citext`), while a learner's own `cards.front` is plain `text` and matches case-sensitively. That asymmetry is deliberate; it is why a case-only edit in Notion (`drive` → `Drive`) never changes what learners see, and it must not be "fixed" without first addressing two migration hazards. Both are recorded in [`docs/backend/ddd-patterns/catalog-user-deck-front-case-asymmetry.md`](backend/ddd-patterns/catalog-user-deck-front-case-asymmetry.md).
