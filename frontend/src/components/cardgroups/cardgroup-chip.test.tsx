...
describe("<CardgroupChip>", () => {
  it("renders the cardgroup name when provided", () => {
    renderWithIntl(<CardgroupChip name="Spanish Vocab" onChangeRequested={vi.fn()} />);
    expect(screen.getByText("Spanish Vocab")).toBeInTheDocument();
  });

  it('renders "Select cardgroup" placeholder when name is null', () => {
    renderWithIntl(<CardgroupChip name={null} onChangeRequested={vi.fn()} />);
  ...
  it("renders placeholder for empty-string name (treated as no name)", () => {
    // `name || t("pickerTitle")` uses truthy coalescing, so "" is treated
    // the same as null and renders the placeholder. Muted style is applied via isMuted = !name.
    renderWithIntl(<CardgroupChip name="" onChangeRequested={vi.fn()} />);
  ...
  it("fires onChangeRequested when the button is clicked", async () => {
    ...
    renderWithIntl(<CardgroupChip name="My Group" onChangeRequested={onChangeRequested} />);
  ...
  it("renders the ChevronDown icon inside the button", () => {
    renderWithIntl(<CardgroupChip name="My Group" onChangeRequested={vi.fn()} />);
  ...
  it("has a descriptive aria-label that includes the cardgroup name when provided", () => {
    renderWithIntl(<CardgroupChip name="Japanese Kanji" onChangeRequested={vi.fn()} />);
    const button = screen.getByRole("button");
  ...
  it("carries the cardgroup-chip test id for locale-independent e2e selection", () => {
    renderWithIntl(<CardgroupChip name="My Group" onChangeRequested={vi.fn()} />);
    expect(screen.getByTestId("cardgroup-chip")).toBe(screen.getByRole("button"));
  });

  it("name span carries the truncate class for overflow handling", () => {
    renderWithIntl(<CardgroupChip name="Very Long Cardgroup Name" onChangeRequested={vi.fn()} />);
